...
const tools: WebMCPTool[] = [
  ...
  { name: 'codex_apply_hardening', title: 'Apply selected deterministic hardening', description: 'After the exact action ids have been reviewed, applies only the in-memory demo action. For every real workspace it returns a native Codex host handoff; Codex must obtain write approval, create a sibling backup, apply the listed JSON cleanup, and submit a refreshed snapshot. It never invents commands, URLs, policies, or instruction text.', annotations: write, inputSchema: { type: 'object', properties: { actionIds: { type: 'array', items: { type: 'string' }, minItems: 1 }, confirm: { type: 'boolean', const: true } }, required: ['actionIds', 'confirm'], additionalProperties: false }, execute: async (input) => { const actionIds = parseCleanupToolInput(input); if (getAccessMode() !== 'demo') return { handoff: dispatchHostHandoff('apply', actionIds) }; const result = await applyBrowserFixes(actionIds); return { appliedActionIds: result.appliedActionIds, skippedActionIds: result.skippedActionIds, backups: result.backups, scan: result.scan } } },
  ...
  { name: 'codex_get_access_scope', title: 'Explain the granted Codex scope', description: 'Returns the active folder boundary, access mode, and privacy guarantees. It never claims whole-system access. Read-only browser scope includes the Codex host handoff contract for any write or current-workspace scan.', annotations: readOnly, inputSchema: emptyInput, execute: () => { const scan = requireScan(); const mode = getAccessMode(); return { scope: scan.scope, platform: scan.platform, privacy: scan.privacy, hostHandoff: mode === 'import' || mode === 'direct' ? buildHostScanHandoff(mode) : null } } },
]
