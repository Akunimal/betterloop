...
export function buildFixPlan(scan: ScanResult) {
  const items = scan.findings.filter((finding) => finding.severity !== 'healthy').map((finding, index) => ({ id: `fix-${index + 1}`, title: finding.title, detail: finding.detail, action: finding.title.includes('Duplicate') ? 'Choose one configuration to keep; do not remove the other until its project usage is confirmed.' : finding.title.includes('disabled') ? 'Confirm whether the disabled entry is intentionally retained before removing it.' : 'Verify the expected executable and configuration before changing any command.' }))
  return { automaticChanges: 0, requiresReview: items.length, rule: 'MCPation never edits configuration without a file-by-file user confirmation.', items }
}
