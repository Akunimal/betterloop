...
for (const tool of ['codex_scan_workspace', 'codex_get_tool_inventory', 'codex_get_findings', 'codex_get_instruction_chain', 'codex_get_workspace_graph', 'codex_explain_finding', 'codex_plan_hardening', 'codex_request_host_handoff', 'codex_submit_host_snapshot', 'codex_apply_hardening', 'codex_verify_workspace', 'codex_get_access_scope']) { const ok = app.includes(tool); console.log(`${ok ? '✓' : '✗'} ${tool}`); failed ||= !ok }
