...
for (const tool of ['mcpation_scan_environment', 'mcpation_get_inventory', 'mcpation_get_findings', 'mcpation_plan_cleanup']) { const ok = app.includes(tool); console.log(`${ok ? '✓' : '✗'} ${tool}`); failed ||= !ok }
