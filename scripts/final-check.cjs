...
check('host MCP server contract', mcp.includes("message.method === 'initialize'") && mcp.includes("message.method === 'tools/list'") && mcp.includes("message.method === 'tools/call'") && mcp.includes("name: 'betterloop_host_status'") && mcp.includes("name: 'betterloop_activation_check'"))
check('host MCP temporary consent gate', mcp.includes("url.pathname === '/activate'") && mcp.includes("url.pathname === '/heartbeat'") && mcp.includes("url.pathname === '/deactivate'") && mcp.includes('visible BetterLoop page') && mcp.includes('if (!origin) return false') && mcp.includes('magic-picker.vercel.app'))
