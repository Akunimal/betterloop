...
assert.match(scan.proposals.find((proposal) => proposal.canApply)?.detail || '', /mcpServers\./)
assert.match(scan.proposals.find((proposal) => proposal.canApply)?.detail || '', /mcp\.json/)
assert.match(scan.proposals.find((proposal) => proposal.canApply)?.detail || '', /canonical .* remains active/)
...
const demoProposal = demo.scan.proposals.find((proposal) => proposal.canApply)
assert.match(demoProposal?.title || '', /extra .* entry from \.mcp\.json/)
assert.match(demoProposal?.detail || '', /canonical .* in \.codex\/config\.toml/)
...
const directActions = direct.removals.filter((item) => item.path === '.mcp.json')
assert.equal(directActions.length, 2)
await grantWriteAccessToConnectedEnvironment()
const directApplied = await applyBrowserFixes(directActions.map((item) => item.actionId))
assert.deepEqual(directApplied.appliedActionIds, directActions.map((item) => item.actionId))
assert.equal(directApplied.skippedActionIds.length, 0)
assert.equal(browserRoot.requests, 1)
assert.equal(Object.keys(JSON.parse(browserMcp.text()).mcpServers).length, 0)
assert.equal(directApplied.backups.length, 1)
